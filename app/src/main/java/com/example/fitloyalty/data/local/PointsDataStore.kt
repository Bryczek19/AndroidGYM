...
class PointsDataStore(
    ...
    companion object {
        ...
        private val NOTIFICATIONS_KEY = intPreferencesKey("notifications")
    }
    ...
    val notificationsFlow: Flow<Boolean> = context.dataStore.data.map { preferences ->
        (preferences[NOTIFICATIONS_KEY] ?: 1) == 1
    }
    ...
    val visitsFlow: Flow<List<String>> = context.dataStore.data.map { preferences ->
        ...
        if (visitsText.isBlank()) {
            emptyList()
        } else {
            visitsText.split(";;").reversed()
        }
    }
    ...
    val rewardsFlow: Flow<List<String>> = context.dataStore.data.map { preferences ->
        ...
        if (rewardsText.isBlank()) {
            emptyList()
        } else {
            rewardsText.split(";;").reversed()
        }
    }
    ...
    suspend fun saveNotifications(enabled: Boolean) {
        ...
        context.dataStore.edit { preferences ->
            preferences[NOTIFICATIONS_KEY] = if (enabled) 1 else 0
        }
    }
    ...
    suspend fun addVisit(userName: String = "Użytkownik") {
        ...
        val exitTime = Date(
            entryTime.time + trainingMinutes * 60 * 1000
        )
        ...
        context.dataStore.edit { preferences ->
            ...
            preferences[VISITS_KEY] =
                if (currentVisits.isBlank()) {
                    newVisit
                } else {
                    "$currentVisits;;$newVisit"
                }
        }
    }
    ...
    suspend fun redeemReward(
        ...
        val newReward =
            "$currentDate — Odebrano: $rewardName — -$cost pkt"
    ...
    suspend fun resetSettings() {
        ...
        context.dataStore.edit { preferences ->
            ...
            preferences[NOTIFICATIONS_KEY] = 1
        }
    }
}